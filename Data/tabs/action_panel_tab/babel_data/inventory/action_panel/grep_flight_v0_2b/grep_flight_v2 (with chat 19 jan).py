...
# Import chat backend (Guillm Integration)
try:
    # Resolve path to modules/morph/guillm.py
    # From: .../modules/action_panel/grep_flight_v0_2b/grep_flight_v2.py
    # To:   .../modules/morph/guillm.py
    current_dir = os.path.dirname(os.path.abspath(__file__))
    morph_dir = os.path.abspath(os.path.join(current_dir, '..', '..', 'morph'))
    
    if morph_dir not in sys.path:
        sys.path.insert(0, morph_dir)
        
    from guillm import OllamaManager
    CHAT_BACKEND_AVAILABLE = True
    
    class ChatBackendAdapter:
        """Adapter to make Guillm's OllamaManager look like the expected ChatBackend"""
        def __init__(self):
            # Load config to get Ollama host
            self.host = "http://localhost:11434"
            try:
                config_path = Path.home() / '.warrior_flow' / 'provider_config.json'
                if config_path.exists():
                    with open(config_path, 'r') as f:
                        config = json.load(f)
                        self.host = config.get('ollama', {}).get('host', self.host)
            except Exception as e:
                print(f"Error loading provider config: {e}")

            self.manager = OllamaManager(host=self.host)
            self.session_type = "task"
            self.context_data = {}
            self.on_response = None
            self.on_error = None
            self.on_status = None
            
        def create_session(self, session_type="task"):
            self.session_type = session_type
            # Guillm doesn't strictly enforce sessions, but we track type
            return "guillm_session"
            
        def switch_session_type(self, session_type):
            self.session_type = session_type
            
        def clear_history(self):
            # Guillm manager is stateless regarding history in this context, 
            # but if we wanted to maintain it we would do it here.
            pass
        ...
        def get_available_models(self):
            return self.manager.refresh_models()
            
        def send_message(self, message, model=None):
            def worker():
                try:
                    if self.on_status: self.on_status("Thinking...")
                    
                    # Construct prompt with context
                    full_prompt = message
                    if self.context_data:
                        ctx_str = json.dumps(self.context_data, indent=2)
                        full_prompt = f"<Context>\n{ctx_str}\n</Context>\n\n{message}"
                    
                    response = self.manager.generate(full_prompt, model=model)
                    
                    if self.on_response: self.on_response(response)
                    if self.on_status: self.on_status("Ready")
                except Exception as e:
    ...
    ChatBackend = ChatBackendAdapter
    
except ImportError as e:
    print(f"Guillm backend not available: {e}")
    ChatBackend = None
...
class GrepSurgicalEngine:
    ...
    def log_debug(self, message: str, level: str = "DEBUG"):
        ...
        try:
            # Go up 3 levels to Morph_v1.2 root, then logs/
...
class BottomPanel(tk.Tk):
    ...
    def _create_chat_tab(self):
        ...
        tk.Button(chat_header, text="[New]", command=self._new_session,
                 ...
                 bd=0, padx=5).pack(side=tk.LEFT, padx=(10, 5))
        ...
        provider_menu = ttk.Combobox(provider_frame, textvariable=self.chat_provider,
                                    values=["Ollama", "Claude", "Gemini", "Morph"],
                                    width=12, state='readonly')
        provider_menu.pack(side=tk.RIGHT)
        provider_menu.bind('<<ComboboxSelected>>', lambda e: self._on_provider_changed())
    ...
    def _toggle_context_view(self):
        """Toggle context view and show current context immediately"""
        current = self.context_view_var.get()
        new_state = not current
        self.context_view_var.set(new_state)
        
        if new_state:
            self.context_btn.config(bg='#2d4d3d', relief=tk.SUNKEN) # visual feedback
            context_str = self._gather_full_context()
            
            self.chat_messages.config(state=tk.NORMAL)
            self.chat_messages.insert(tk.END, f"\n📋 CURRENT CONTEXT (Active):\n")
            self.chat_messages.insert(tk.END, f"{'='*30}\n")
            self.chat_messages.insert(tk.END, context_str + "\n")
            self.chat_messages.insert(tk.END, f"{'='*30}\n")
            self.chat_messages.config(state=tk.DISABLED)
            self.chat_messages.see(tk.END)
        else:
            self.context_btn.config(bg=self.config.BG_COLOR, relief=tk.RAISED)
            self.chat_messages.config(state=tk.NORMAL)
    ...
    def _create_inventory_tab(self):
        ...
        inventory_notebook = ttk.Notebook(inventory_tab)
        inventory_notebook.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)

        # Sub-tab 1: Files (File Tree) - NEW
        self._create_file_tree_tab(inventory_notebook)

        # Sub-tab 2: Provisions (from backup_manifest.json)
        self._create_provisions_subtab(inventory_notebook)

        # Sub-tab 3: Stash (stash_script controls)
        self._create_stash_subtab(inventory_notebook)

        # Sub-tab 4: Projects (shared projects)
        self._create_projects_subtab(inventory_notebook)
    ...
    def _gather_full_context(self) -> str:
        ...
        try:
            ...
            # 1. Core Target Context
            target = self.target_var.get()
            pattern = self.pattern_var.get()
            context_parts.append(f"Target: {target or 'Not set'}")
            context_parts.append(f"Pattern: {pattern or 'None'}")
            
            # 2. Action Panel Context (Active Profile, Last Tool)
            profile = getattr(self, 'active_profile', 'Default')
            context_parts.append(f"Active Profile: {profile}")
            
            if hasattr(self, 'last_tool_context') and self.last_tool_context:
                lt = self.last_tool_context
                context_parts.append(f"Last Tool Action: {lt.get('action')} -> {lt.get('status')}")
                context_parts.append(f"Tool Context: {lt.get('raw_text')}")
            
            # 3. Morph/Guillm Context (Simulated or Read)
            # If we had access to guillm.py output or scan profiles, we'd add it here
            context_parts.append("--- Morph Context ---")
            if target and os.path.exists(target):
                context_parts.append(f"Target Type: {'Directory' if os.path.isdir(target) else 'File'}")
                if os.path.isfile(target):
                    size = os.path.getsize(target)
                    context_parts.append(f"Size: {size} bytes")
                    # Peek at content (first few lines)
                    try:
            ...
            # 4. System/Log Context
            log_path = Path(__file__).parent.parent.parent.parent / "logs" / "unified_traceback.log"
            context_parts.append(f"Log Path: {log_path}")
            
            return "\n".join(context_parts)
        except Exception as e:
            self.engine.log_debug(f"Error gathering context: {e}", "ERROR")
    ...
    def _send_chat_message(self):
        ...
        if self.context_view_var.get():
            ...
            # Also show context in chat output for user visibility
            self._add_chat_response(f"📋 Context attached:\n{context_str}")
        ...
        # Add gathered context to backend context as well
        if context_str:
        ...
        # Ensure CLI Working Directory is set for tools
        context_data["cwd"] = self._get_cli_working_directory()

        self.chat_backend.update_context(context_data)

        # Send message
        self.chat_backend.send_message(final_message, model=model)
        ...
        try:
            ...
            if sess_file.exists():
                ...
                if self.chat_backend:
                    # We might want to tell the backend about this ID, 
                    # but ChatBackendAdapter is simple.
                    pass
        except Exception as e:
    ...
    def _new_session(self):
        ...
        try:
            ...
            self._clear_chat()
            self.chat_messages.config(state=tk.NORMAL)
            self.chat_messages.insert(tk.END, f"[{datetime.now().strftime('%H:%M:%S')}] System: New Session Started ({new_id})\n")
            self.chat_messages.config(state=tk.DISABLED)
            
            # Reset session selector
            self.chat_session_select.set('')
    ...
    def _update_available_models(self):
        ...
        if provider == "Ollama":
            ...
            if not models:
                models = ["Install Ollama", "llama3", "mistral", "gemma"]
        ...
        self._add_traceback(f"📋 Loaded {len(models)} models for {provider}", "INFO")
    ...
    def _expand_panel(self):
        ...
        # Calculate new position - total height includes expanded content + collapsed panel
        total_height = self.config.EXPANDED_HEIGHT + self.config.PANEL_HEIGHT
        target_y = screen_height - total_height
        self.engine.log_debug(f"  Target position: y={target_y}, total_height={total_height} (expanded={self.config.EXPANDED_HEIGHT} + collapsed={self.config.PANEL_HEIGHT})")
    ...
    def _load_agent_profiles(self) -> Dict:
        ...
        try:
            config_path = self.version_root / "Modules" / "config" / "profiles.json"
            if config_path.exists():
                with open(config_path, 'r') as f:
                    data = json.load(f)
            ...
            self.engine.log_debug(f"Error loading agent profiles: {e}", "ERROR")
            return {}

    def _save_agent_profiles(self, profiles: Dict):
        """Save agent profiles to config"""
        try:
            config_dir = self.version_root / "Modules" / "config"
            config_dir.mkdir(parents=True, exist_ok=True)
            config_path = config_dir / "profiles.json"
            
            # Load existing to preserve structure
            data = {"version": "1.0", "active_profile": "orchestrator", "profiles": {}}
            if config_path.exists():
                with open(config_path, 'r') as f:
                    data = json.load(f)
            ...
            self.engine.log_debug(f"Error saving agent profiles: {e}", "ERROR")

    def _create_profiles_tab(self, notebook):
        """Create Profiles configuration tab"""
        ...
        tk.Label(left_frame, text="Agent Profiles", bg='#1e1e1e', fg='#4ec9b0').pack(anchor=tk.W)
        ...
        profile_list = tk.Listbox(list_frame, bg='#2a2a2a', fg='#d4d4d4', 
        ...
        # Right: Details (read-only preview for now)
        right_frame = tk.Frame(paned, bg='#1e1e1e')
        paned.add(right_frame, weight=2)

        tk.Label(right_frame, text="Profile Details", bg='#1e1e1e', fg='#4ec9b0').pack(anchor=tk.W)
        
        details_text = scrolledtext.ScrolledText(right_frame, bg='#2a2a2a', fg='#d4d4d4', height=15)
        ...
        for pid, pdata in profiles.items():
            ...
            icon = pdata.get('icon', '')
            profile_list.insert(tk.END, f"{icon} {name} ({pid})")
        ...
        def on_select(event):
            ...
            idx = selection[0]
            # Get key from index (assuming order preserved)
            pid = list(profiles.keys())[idx]
            pdata = profiles[pid]
            
            details_text.delete(1.0, tk.END)
            details_text.insert(tk.END, json.dumps(pdata, indent=2))
        ...
        def delete_profile():
            ...
            if messagebox.askyesno("Delete", f"Delete profile '{pid}'?"):
                del profiles[pid]
                self._save_agent_profiles(profiles)
                refresh_list()
                details_text.delete(1.0, tk.END)
        ...
        tk.Button(btn_frame, text="New Profile", command=new_profile, 
                 bg='#2d7d46', fg='white').pack(side=tk.LEFT, padx=5)
        
        tk.Button(btn_frame, text="Delete", command=delete_profile, 
                 bg='#a54242', fg='white').pack(side=tk.LEFT, padx=5)
        
        tk.Button(btn_frame, text="Save Edits", command=lambda: self._save_from_text(details_text, profiles, refresh_list),
                 bg='#0e639c', fg='white').pack(side=tk.RIGHT, padx=5)

    def _save_from_text(self, text_widget, profiles_dict, refresh_cb):
        try:
            ...
            # Find which profile this matches (by scanning values or we need to track selection)
            # For now, simplistic: if we are editing, we are likely editing the selected one.
            # But 'profiles_dict' is the full dict. We need to know WHICH key to update.
            # A better way is to parse the 'name' or just trust the user didn't change the structure too much.
            # Let's just update the profile list.
            
            # Actually, the user might be editing the JSON of a single profile.
            # We need to know which key it corresponds to.
            # Let's assume the user selects, edits, then clicks save.
            # We can iterate profiles_dict and see which one matches the 'name' or 'agent_type' 
            # if we didn't track the key.
            # BUT, to be safe, let's just add a "Save" button logic that requires selection state tracking.
            # For simplicity in this inserted method, I'll rely on the user to not break keys.
            
            # Wait, I can't easily track selection variable inside this helper.
            # I'll just save the JSON back if it's valid.
            # Actually, I'll implement a simple "Save" that re-reads the text and updates the *currently selected* profile in the parent scope.
            # But I don't have access to 'profile_list' here easily unless I pass it.
            
            # Let's simplify: The "Save Edits" button logic is inside _create_profiles_tab scope.
            pass 
        except Exception as e:
    ...
    def _create_tool_profiles_tab(self, notebook):
        ...
        tk.Button(
            btn_frame,
            text="📜 Manage Scripts",
            command=lambda: notebook.select(3), # Tab 4: Custom Scripts
            bg='#2d2d2d',
            fg='#d4d4d4',
            width=15
        ).pack(side=tk.LEFT, padx=5)
        ...
        tk.Button(
            ...
            command=lambda: notebook.select(2), # Tab 3: Profiles
            bg='#2d2d2d',
            fg='#d4d4d4',
            width=15
        ).pack(side=tk.LEFT, padx=5)
    ...
    def _load_debug_tool_profiles(self) -> Dict[str, Any]:
        ...
        config_dir = self.version_root / ".docv2_workspace" / "config"
        config_dir.mkdir(parents=True, exist_ok=True)
        config_path = config_dir / "custom_tools.json"
        
        if not config_path.exists():
            # Fallback/Migration: Check legacy path
            legacy_path = Path.home() / '.warrior_flow' / 'custom_tools.json'
    ...
    def _refresh_projects(self):
        ...
        try:
            ...
            if not projects_found:
                ...
                self.projects_listbox.insert(tk.END, "  Use warrior_gui [New Project] to create provisions")
