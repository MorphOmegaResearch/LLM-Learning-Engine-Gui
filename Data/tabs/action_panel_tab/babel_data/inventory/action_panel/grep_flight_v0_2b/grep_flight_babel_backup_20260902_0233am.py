...
class GrepSurgicalEngine:
    ...
    def log_debug(self, message: str, level: str = "DEBUG"):
        ...
        try:
            ...
            # parents[3] is Babel_v01a root
            log_dir = Path(__file__).resolve().parents[3] / "babel_data" / "logs"
            if log_dir.exists():
...
class BottomPanel(tk.Tk):
                        ...
                        if current_size > self.last_shared_log_size:
                            with open(self.shared_log_path, 'r') as f:
                                ...
                                for line in new_lines:
                                    line = line.strip()
                                    if line:
                                        # Only process logs from other modules to avoid loops
                                        if "[grep_flight]" not in line:
    ...
    def _create_tasks_tab(self):
        ...
        plan_module_frame = tk.Frame(tasks_tab, bg='#252525', pady=5)
        plan_module_frame.pack(fill=tk.X, padx=10, pady=(0, 10))
        ...
        # Invocation Display (Parent Script)
        ...
        # --- TAXONOMY FILTER SUITE ---
        filter_suite_frame = ttk.LabelFrame(tasks_tab, text="Taxonomy Filter Suite")
        filter_suite_frame.pack(fill=tk.X, padx=10, pady=(0, 10))

        # Create a grid for taxonomy toggles
        taxonomy_grid = tk.Frame(filter_suite_frame, bg=self.config.BG_COLOR)
        taxonomy_grid.pack(fill=tk.X, padx=5, pady=5)
        ...
        for i, cat in enumerate(self.system_categories):
            ...
            cb.grid(row=r, column=c, sticky=tk.W, padx=5)

        # Filter controls (Original Type/Status)
        ...
        if self.app_ref:
            tk.Button(task_actions, text="→ App Tasks", command=self._open_tasks_tab,
                     bg=self.config.ACCENT_COLOR, fg='black',
                     font=('Arial', 10), padx=10).pack(side=tk.LEFT, padx=5)

            planner_btn = tk.Button(task_actions, text="→ Planner", command=self._open_planner_tab,
                     bg='#613d8c', fg='white',
                     font=('Arial', 10), padx=10)
            planner_btn.pack(side=tk.LEFT, padx=5)
            if not hasattr(self.app_ref, 'planner_panel'):
                planner_btn.config(state='disabled', text="→ Planner [N/A]")
    ...
    def _create_chat_tab(self):
        ...
        provider_menu = ttk.Combobox(provider_frame, textvariable=self.chat_provider,
                                    values=["Ollama", "Claude", "Gemini", "Morph", "Manual"],
                                    width=12, state='readonly')
    ...
    def _toggle_context_view(self):
        """Toggle context view and show current context immediately (Morph Effect)"""
        ...
        if new_state:
            ...
            # Traceback Event
            log_to_traceback(EventCategory.CHAT, "morph_context_flip", 
                           {"target": self.target_var.get()}, 
                           {"status": "Payload Generated", "clipboard": "Synced"})
        else:
            self.context_btn.config(bg=self.config.BG_COLOR, relief=tk.RAISED)
            self.chat_messages.config(state=tk.NORMAL)
            self.chat_messages.insert(tk.END, "\n📋 Morph Context: DISABLED\n")
            self.chat_messages.config(state=tk.DISABLED)
            self.chat_messages.see(tk.END)
    ...
    def _update_project_context(self):
        ...
        # Manifests are in filesync_output/manifests/ (relative to repo root usually)
        # Babel root is self.babel_root
        manifest_dir = self.babel_root / "filesync_output" / "manifests"
        ...
        try:
            ...
            # Look for primary project or inference
            projects = data.get('projects', {})
            if projects:
                # Get the first project (or most significant)
                # Usually there's one main cluster for the repo
                ...
                self.project_context_var.set(f"Project Context: {p_type} - {p_subtype} ({conf}% confidence)")
            else:
            ...
            self.project_context_var.set("Project Context: Error loading")
    ...
    def _gather_full_context(self) -> str:
        """Gather comprehensive context for chat interaction (Morph Effect)"""
        try:
            context_parts = []
            ...
            # 5. Morph Suggestions (Low Resource Engine)
    ...
    def _send_chat_message(self):
        ...
        context_data = {
            ...
            "morph_profile": self.morph_profile_var.get() if self.chat_provider.get() == "Morph" else None,
            "morph_workflow": self.morph_workflow_var.get() if self.chat_provider.get() == "Morph" else None,
            "session_id": self.chat_session_select.get() if self.chat_session_select.get() and self.chat_session_select.get() != "Select Session" else "1",
            "tool_switches": {
    ...
    def _new_session(self):
        ...
        try:
            ...
            # Create session file
            session_file = self.version_root / "sessions" / f"session_{new_id}.txt"
            session_file.parent.mkdir(parents=True, exist_ok=True)
            with open(session_file, 'w') as f:
                f.write(f"[{datetime.now().strftime('%H:%M:%S')}] System: New Session Started ({new_id})\n")

            # Reset session selector and select the new one
            self._refresh_session_list()
            self.chat_session_select.set(new_id)
            self._on_session_selected() # Trigger context update
    ...
    def _refresh_morph_profiles(self):
        ...
        try:
            ...
            profiles = [f.stem for f in Path(prof_dir).glob("*.json")]
            self.morph_profile_menu['values'] = sorted(profiles)
    ...
    def _refresh_morph_workflows(self):
        ...
        try:
            current_dir = os.path.dirname(os.path.abspath(__file__))
            prof_path = os.path.abspath(os.path.join(current_dir, '..', '..', 'morph', 'morph_profiles', f"{profile_id}.json"))
    ...
    def _create_workflow_config_tab(self, notebook):
        ...
        self._refresh_workflow_config = refresh_workflow_list
