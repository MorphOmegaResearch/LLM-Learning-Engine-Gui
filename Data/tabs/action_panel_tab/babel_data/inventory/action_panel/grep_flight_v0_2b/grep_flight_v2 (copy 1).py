...
class BottomPanel(tk.Tk):
    ...
    def _log_to_json(self, entry_type: str, data: dict):
        ...
        try:
            ...
            self.engine.log_debug(f"Failed to write to log.json: {e}", "ERROR")

    def _update_backup_manifest(self, original_file: str, backup_file: str, backup_info: dict):
        """Update the backup manifest with new backup entry"""
        try:
            ...
            # Get absolute paths
            original_abs = str(Path(original_file).resolve())
            ...
            if len(manifest[original_abs]) > 50:
                manifest[original_abs] = manifest[original_abs][-50:]
            ...
            self.engine.log_debug(f"Failed to update backup manifest: {e}", "ERROR")
    ...
    def _get_backups_for_file(self, target_file: str) -> list:
        ...
        try:
            ...
            # Filter to only existing backups
            existing_backups = []
    ...
    def _show_restore_menu(self):
        """Show dropdown menu with available backups for current target"""
        target = self.target_var.get().strip()

        # Get file from target + pattern
        if not target:
            self._add_traceback("❌ No target set", "ERROR")
            messagebox.showerror("Error", "No target file set")
            return
        ...
        if not os.path.isfile(target):
            self._add_traceback(f"❌ Not a file: {target}", "ERROR")
            messagebox.showerror("Error", f"Not a file: {target}")
            return
        ...
        if not backups:
            self._add_traceback("ℹ️ No backups found for this file", "INFO")
            messagebox.showinfo("No Backups", f"No backups found for:\n{Path(target).name}")
            return
    ...
    def _restore_from_backup(self, backup_path: str, target_path: str):
        ...
        try:
            ...
            # Create Bug-marked version of current file
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            bug_name = f"{target_p.stem}_Bug_{timestamp}{target_p.suffix}"
            bug_path = target_p.parent / bug_name

            # Copy current file to Bug version
            shutil.copy2(target_path, bug_path)

            # Copy backup to target (restoring it)
            shutil.copy2(backup_path, target_path)

            # Log the restore operation
            self._log_to_json("restore", {
                "action": "restore_completed",
                "target_file": str(target_p.resolve()),
                "backup_file": backup_path,
                "bug_marked_file": str(bug_path.resolve()),
            ...
            self._add_traceback(
                ...
                f"   From backup: {Path(backup_path).name}\n"
            ...
            messagebox.showinfo(
                ...
                f"Restored file: {target_p.name}\n"
                f"Bug-marked version: {bug_name}\n\n"
                f"The previous version has been saved with 'Bug' in the name."
            )
            ...
            messagebox.showerror("Restore Failed", f"Failed to restore from backup:\n{e}")
