...
class BrainMapVisualizer(tk.Toplevel):
    ...
    def __init__(self, parent, style, root_app):
        ...
        self.animation_running = False  # Off by default — user must enable
        ...
        # Draw static frame without animating (animation starts only when toggled on)
        self._render_brain()
    ...
    def _create_ui(self):
        ...
        # Animation toggle (off by default to conserve resources)
        ...
        self.anim_btn.pack(side=tk.RIGHT, padx=5)
    ...
    def _toggle_animation(self):
        """Toggle animation on/off to manage CPU/GPU resource use."""
        if self.animation_running:
            self.animation_running = False
            self.anim_btn.config(text="▶ Enable Animation", bg='#333333', fg='#aaaaaa')
        else:
            self._start_animation()
            self.anim_btn.config(text="⏸ Pause Animation", bg='#1a1a2e', fg='#56d4e8')

    def stop_animation(self):
        """Stop animation loop (called externally when tab is hidden)."""
        self.animation_running = False
