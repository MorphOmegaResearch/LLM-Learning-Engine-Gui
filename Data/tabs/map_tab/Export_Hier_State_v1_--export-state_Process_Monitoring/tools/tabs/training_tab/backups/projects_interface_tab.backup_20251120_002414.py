...
class ProjectsInterfaceTab(ChatInterfaceTab):
    ...
    _projects_interface_tab.backup_20251120_002414_debug_logger = get_debug_logger("projects_interface_tab.backup_20251120_002414")
    ...
    @debug_ui_event(_projects_interface_tab.backup_20251120_002414_debug_logger)
    def create_ui(self):
    ...
    def _build_projects_sidebar(self, parent):
        ...
        # Make project tree (row 1) and training grounds tree (row 3) resizable
        try:
        ...
        tg_hdr.grid(row=2, column=0, sticky=tk.EW, padx=8, pady=(0,4))
        ttk.Label(tg_hdr, text='🎯 Training Grounds', style='CategoryPanel.TLabel').pack(side=tk.LEFT)

        # Training Grounds tree: Type -> Variant -> Chats
        tg_wrap = ttk.Frame(parent, style='Category.TFrame')
        tg_wrap.grid(row=3, column=0, sticky=tk.NSEW, padx=8, pady=(0,8))
        ...
        actions.grid(row=4, column=0, sticky=tk.EW, padx=8, pady=(0,4))
        for c in range(2):
        ...
        rag_bar.grid(row=5, column=0, sticky=tk.EW, padx=8, pady=(0,4))
