...
from config import (
    ...
    apply_evaluation_stats,
    get_stat,
)
from logger_util import get_tab_logger, get_log_file_path
from xp_calculator import process_stat_improvements
...
class RunnerPanel:
        ...
        try:
            ...
            if profile_name:
                ...
                try:
                    ...
                    type_schema_name = getattr(self.training_tab_instance, 'type_schema_selected_var', None)
                    ...
                    if type_schema_name and type_schema_name.get():
                        summary_lines.append(f"Type-Schema: {type_schema_name.get()}")
                except Exception:
                    pass
    ...
    def _run_training_process(self, command, env_vars):
        ...
        try:
            ...
            if self.process.returncode != 0:
                ...
                # Sync bundles after successful training
                try:
                    from registry.bundle_loader import sync_bundles_from_profiles
                    self.append_runner_output("\n=== Syncing bundle registry ===\n")
                    summary = sync_bundles_from_profiles(verbose=False)
                    if summary.get('error'):
                        self.append_runner_output(f"⚠️ Bundle sync error: {summary['error']}\n")
                    else:
                        updated = summary.get('bundles_updated', 0)
                        created = summary.get('bundles_created', 0)
                        self.append_runner_output(f"✓ Bundles synced ({created} created, {updated} updated)\n")
                except Exception as e:
                    self.append_runner_output(f"⚠️ Bundle sync failed: {e}\n")
    ...
    def _auto_run_evaluation(self, as_baseline: bool):
        ...
        if as_baseline:
            ...
            # Update hierarchical stats + XP
            self._apply_stats_and_xp(model_name, aggregated)
    ...
    def _apply_stats_and_xp(self, variant_id: str, aggregated: Dict[str, Any]) -> None:
        try:
            stat_updates: Dict[str, float] = {}

            pass_rate = aggregated.get('pass_rate_percent')
            rate = self._parse_percent(pass_rate)
            if rate is not None:
                stat_updates['performance.accuracy.evaluation_pass_rate'] = rate

            per_skill = aggregated.get('per_skill') or {}
            for skill_name, data in per_skill.items():
                per_rate = None
                total = data.get('total')
                passed = data.get('passed')
                if isinstance(total, (int, float)) and total:
                    per_rate = float(passed or 0) / float(total)
                else:
                    per_rate = self._parse_percent(data.get('pass_rate_percent'))
                if per_rate is not None:
                    path = f"skills.{skill_name}.evaluation_pass_rate"
                    stat_updates[path] = per_rate

            if not stat_updates:
                return

            old_stats = {path: get_stat(variant_id, path, 0.0) for path in stat_updates}
            result = apply_evaluation_stats(variant_id, {'stat_updates': stat_updates}, update_grades=True)
            if result and not result.get('success', True):
                self.append_runner_output(f"Stat update notice: {result.get('message', 'unknown')}\n")

            stat_changes: Dict[str, Tuple[float, float]] = {}
            for path, old_value in old_stats.items():
                new_value = get_stat(variant_id, path, old_value)
                if new_value is None:
                    continue
                if float(new_value) != float(old_value):
                    stat_changes[path] = (float(old_value or 0.0), float(new_value))

            if stat_changes:
                xp_result = process_stat_improvements(variant_id, stat_changes)
                xp_awarded = xp_result.get('xp_awarded', 0)
                self.append_runner_output(
                    f"Stats updated ({len(stat_changes)} paths). XP +{xp_awarded}\n")
        except Exception as exc:
            log_message(f"RUNNER: Failed to apply evaluation stats for {variant_id}: {exc}")
