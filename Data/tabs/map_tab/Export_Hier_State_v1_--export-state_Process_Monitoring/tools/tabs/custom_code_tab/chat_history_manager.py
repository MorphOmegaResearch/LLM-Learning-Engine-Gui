...
class ChatHistoryManager:
    ...
    def __init__(self, history_dir: Optional[Path] = None):
        ...
        if history_dir is None:
            self.history_dir = TRAINING_DATA_DIR / "ChatHistories"
        else:
    ...
    def _normalize_model_name(model_name: str) -> str:
        """
        Extract clean model name from full path to prevent path traversal issues

        Args:
            model_name: Model name or full path

        Returns:
            Clean model name safe for use in filename
        """
        if '/' in model_name or '\\' in model_name:
            # Extract filename without extension from full path
            model_name = Path(model_name).stem
        # Clean any remaining path separators and special chars
    ...
    def save_conversation(
        ...
        Args:
            model_name: Name of the model used (can be full path)
        ...
        """
        try:
            # Normalize model name to prevent path issues
            normalized_model = self._normalize_model_name(model_name)

            # Generate session ID if not provided
            if session_name is None:
                timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                session_name = f"{normalized_model}_{timestamp}"
            else:
                # Clean session name
            ...
            print(f"ChatHistoryManager DEBUG: saved conversation to {session_file}")
