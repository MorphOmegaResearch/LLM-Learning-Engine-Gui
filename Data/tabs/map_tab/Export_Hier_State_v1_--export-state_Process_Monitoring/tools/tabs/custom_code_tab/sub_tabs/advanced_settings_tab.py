...
class AdvancedSettingsTab(BaseTab):
    ...
    def __init__(self, parent, root, style, parent_tab):
        ...
        self._mainloop_ready = False  # Guard to prevent refresh during mainloop startup
    ...
    def create_ui(self):
        ...
        # Mark mainloop as ready after initial UI creation
        if not self._mainloop_ready:
            # Defer setting the flag slightly to ensure mainloop has fully started
            try:
                self.root.after(1500, lambda: setattr(self, '_mainloop_ready', True))
                log_message("ADV_SETTINGS: Scheduled mainloop ready flag for 1.5s from now")
            except Exception:
                pass
        # Bind to Training/Support changes to refresh header pill live
        try:
            if not getattr(self, '_ts_event_bound', False):
                self.root.bind("<<TrainingSupportChanged>>", lambda e: self.refresh())
                self._ts_event_bound = True
        except Exception:
            pass
        try:
            if not getattr(self, '_tm_event_bound', False):
                self.root.bind("<<TrainingModeChanged>>", lambda e: self.refresh())
    ...
    def create_header(self):
        ...
        # Info label: include mode-enabled + explicitly enabled in settings
        try:
            mode_enabled = set(self.MODE_ADVANCED_MAPPINGS.get(self.current_mode, {}).get('enabled_systems', []))
        except Exception:
            mode_enabled = set()
        try:
            explicit_enabled = set()
            ...
            explicit_enabled = set()
        enabled_count = len(mode_enabled.union(explicit_enabled))
        ttk.Label(
            header_frame,
            text=f"({enabled_count} systems enabled)",
            font=("Arial", 9),
            foreground='#888888',
            style='Config.TLabel'
        ).pack(side=tk.LEFT, padx=(0, 10))

        # Training/Support status pill (mirrors Chat indicators)
        try:
            t_on = False; s_on = False
            chat = getattr(self.parent_tab, 'chat_interface', None)
            if chat:
                t_on = bool(getattr(chat, 'training_mode_enabled', False))
                s_on = bool(getattr(chat, 'backend_settings', {}).get('training_support_enabled', False))
            self.training_support_pill = ttk.Label(
                header_frame,
                text=f"🏋️ Training: {'ON' if t_on else 'OFF'} | Support: {'ON' if s_on else 'OFF'}",
                font=("Arial", 9, "bold"),
                foreground=('#00ff00' if t_on else '#888888'),
                style='Config.TLabel'
            )
            self.training_support_pill.pack(side=tk.LEFT, padx=(0, 10))
        except Exception:
            pass
    ...
    def refresh(self):
        """Refresh the settings UI and header indicators"""
        # Prevent refresh during mainloop startup to avoid X11 resource exhaustion
        if not self._mainloop_ready:
            log_message("ADV_SETTINGS: Refresh blocked - mainloop not ready yet")
            return
