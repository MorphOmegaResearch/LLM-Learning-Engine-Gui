...
try:
    from code_change_watcher import (
        ...
        set_active_version as watcher_set_active_version,
    )
...
if not WATCHER_AVAILABLE:
    ...
    def watcher_duplicate_version(*_args, **_kwargs):
        return None
    def watcher_set_active_version(*_args, **_kwargs):
        return None
...
class SettingsTab(BaseTab):
    ...
    def __init__(self, parent, root, style, main_gui=None, tab_instances=None):
        ...
        # Planner/TODO popup tracking
        self._todo_content_pane = None
        self._planner_dock_frame = None
        self._active_popup_project = None
    ...
    def _create_planner_dock_section(self, parent, project_context: Optional[str] = None):
        ...
        ttk.Button(
            ...
            text='▲ Collapse Dock',
            style='Select.TButton',
            command=lambda: self._set_planner_dock_visibility(
                False,
                project_context or getattr(self, '_planner_project_context', None)
            )
        ).pack(side=tk.RIGHT)
    ...
    def _set_planner_dock_visibility(self, visible: bool, project_name: Optional[str] = None):
        """
        Centralized handler to show/hide the planner dock and refresh the popup in-place.
        """
        if not hasattr(self, '_planner_dock_visible'):
            self._planner_dock_visible = False
            self._planner_project_context = None

        previous_state = getattr(self, '_planner_dock_visible', False)
        self._planner_dock_visible = bool(visible)
        self._planner_dock_expanded = bool(visible)
        ...
        log_message(
            f"SETTINGS: Planner dock visibility change requested: "
            f"{previous_state} -> {self._planner_dock_visible} (project={project_name})"
        )

        fallback_project = (
            project_name
            or getattr(self, '_planner_project_context', None)
            or getattr(self, '_active_popup_project', None)
        )
        try:
            rendered = self._render_planner_dock(fallback_project)
        except Exception as exc:
            rendered = False
            log_message(f"SETTINGS: Planner dock render error: {exc}")
            if _settings_debug_logger:
                _settings_debug_logger.error("Planner dock render failure", exc_info=exc)

        if rendered:
            # Collapse requests still need a quick refresh so the UI visibly updates.
            if not getattr(self, '_planner_dock_visible', False):
                self._refresh_active_todo_popup(fallback_project)
            return

        # Fallback: rebuild popup if in-place refresh is unavailable
        self._refresh_active_todo_popup(fallback_project)

    def _refresh_active_todo_popup(self, project_name: Optional[str]) -> None:
        """Close and reopen the TODO popup to reflect dock layout changes."""
        if not getattr(self, 'todo_popup_active', False):
            log_message("SETTINGS: TODO popup inactive; dock change will apply on next open")
            return
        active_popup = getattr(self, '_active_todo_popup_window', None)
        if not active_popup:
            log_message("SETTINGS: No active popup reference found for dock refresh")
            return
        try:
            active_popup.destroy()
            log_message("SETTINGS: Closed TODO popup for dock refresh")
        except Exception as exc:
            log_message(f"SETTINGS: Error closing popup during dock refresh: {exc}")
        try:
            if project_name:
            ...
            log_message(
                "SETTINGS: Reopened TODO popup after dock visibility change "
                f"(visible={self._planner_dock_visible})"
            )
        except Exception as exc:
            log_message(f"SETTINGS: Error reopening popup after dock refresh: {exc}")
    ...
    def _open_planner_agent_dock(self, project_name: Optional[str] = None):
        ...
        """
        self._set_planner_dock_visibility(
            not getattr(self, '_planner_dock_visible', False),
            project_name
        )
    ...
    def _get_chat_interface_tab(self):
        ...
        """
        try:
            custom_code = self._get_custom_code_tab()
            if custom_code and hasattr(custom_code, 'chat_interface_tab'):
    ...
    def _get_custom_code_tab(self):
        """Return the Custom Code tab reference if available."""
        try:
            tab = getattr(self, '_cached_custom_code_tab', None)
            if tab:
                return tab

            # Check tab_instances mapping
            tab_instances = getattr(self, 'tab_instances', None)
            if isinstance(tab_instances, dict):
                tab = tab_instances.get('Custom Code')
                if tab:
                    self._cached_custom_code_tab = tab
                    return tab

            # Check main_gui reference
            main_gui = getattr(self, 'main_gui', None)
            if main_gui and hasattr(main_gui, 'custom_code_tab'):
                tab = getattr(main_gui, 'custom_code_tab')
                if tab:
                    self._cached_custom_code_tab = tab
                    return tab
            ...
            if root and hasattr(root, 'custom_code_tab'):
                tab = getattr(root, 'custom_code_tab')
                if tab:
                    self._cached_custom_code_tab = tab
                    return tab
        except Exception as exc:
            log_message(f"SETTINGS: Failed to resolve Custom Code tab reference: {exc}")
        return None
    ...
    def _mount_planner(self) -> tuple[bool, str]:
        ...
        try:
            ...
            custom_code = self._get_custom_code_tab()
            if not custom_code:
                return False, "Failed to access Custom Code tab. Please open the Custom Code tab once and retry."
    ...
    def _show_planner_tools_config(self):
        ...
        try:
            custom_code = self._get_custom_code_tab()
            if custom_code and hasattr(custom_code, '_show_agent_tools_popup'):
                custom_code._show_agent_tools_popup('planner')
                return
            messagebox.showinfo(
                "Planner Tools",
                "Planner tools are configured via planner_skilled.json.\n\n"
                "Please open the Custom Code tab at least once so planner tool toggles become available.",
                parent=self.root
            )
        except Exception as e:
    ...
    def _show_planner_conformers_popup(self):
        ...
        try:
            custom_code = self._get_custom_code_tab()
            if custom_code and hasattr(custom_code, '_show_conformer_popup'):
                custom_code._show_conformer_popup('planner')
                return
            messagebox.showinfo(
                "Planner Conformers",
                "Conformer events require the Custom Code tab. Please open it first and retry.",
                parent=self.root
            )
        except Exception as e:
    ...
    def _show_planner_tasks_popup(self):
        ...
        try:
            custom_code = self._get_custom_code_tab()
            if custom_code and hasattr(custom_code, '_show_agent_tasks_popup'):
                custom_code._show_agent_tasks_popup('planner')
                return
            messagebox.showinfo(
                "Planner Tasks",
                "Planner tasks popup requires the Custom Code tab. Please open it first and retry.",
                parent=self.root
            )
        except Exception as e:
...
'''
    ...
    def show_todo_popup(self, project_name: str = None, prefer_project: bool = False, auto_open_create: bool = False):
        """[FC-TODO-MANAGER] Show main todo popup.

        Feature Code: FC-TODO-MANAGER
        Location: settings_tab.py:4400 (TODO Manager main popup)
        Plan: Plans/Todo_Manager_Task_Assignment_Plan.md
        Status: Active - TD-001 (roster integration complete, dispatch pending)
        ...
        self._todo_content_pane = content_pane
        self._active_popup_project = project_name
        if not hasattr(self, '_planner_dock_visible'):
            self._planner_dock_visible = False
        self._render_planner_dock(project_name)
        ...
        try:
            ...
            # Check if planner dock is visible
            dock_visible = getattr(self, '_planner_dock_visible', False)
            if dock_visible:
        ...
        try:
            ...
            ttk.Button(
                ...
                text="🔗 Link to Plan",
                style='Action.TButton',
                command=_link_plan_cb
            ).grid(row=0, column=0, sticky=tk.EW, padx=(0, 4))
            ttk.Button(
                button_row,
                text="🔀 Merge Plans",
                style='Select.TButton',
                command=_merge_plans_cb
            ).grid(row=0, column=1, sticky=tk.EW, padx=(4, 0))

            # Manual action shortcuts reinstated beneath link/merge controls
            actions_row = ttk.Frame(button_row)
            actions_row.grid(row=1, column=0, columnspan=2, sticky=tk.EW, pady=(6, 0))
            for i in range(5):
                actions_row.columnconfigure(i, weight=1)
            ttk.Button(actions_row, text="➕ Create Todo", command=create_cb, style='Action.TButton').grid(row=0, column=0, padx=3, sticky=tk.EW)
            ttk.Button(actions_row, text="✔ Mark Complete", command=mark_cb, style='Select.TButton').grid(row=0, column=1, padx=3, sticky=tk.EW)
            ttk.Button(actions_row, text="📄 Open File", command=edit_cb, style='Select.TButton').grid(row=0, column=2, padx=3, sticky=tk.EW)
            ttk.Button(actions_row, text="🗑 Delete Todo", command=delete_cb, style='Select.TButton').grid(row=0, column=3, padx=3, sticky=tk.EW)
            ttk.Button(actions_row, text="💾 Save (Title/Priority)", command=save_cb, style='Action.TButton').grid(row=0, column=4, padx=3, sticky=tk.EW)

            log_message("TODO POPUP: Link/Merge + manual action rows created successfully")
        except Exception as e:
    ...
    def show_project_todo_popup(self, project_name):
        """[FC-PROJECT-TODO] Show project-specific todo popup - completely separate from main todos.

        Feature Code: FC-PROJECT-TODO
        Location: settings_tab.py:5694 (Project TODO Manager)
        Plan: Plans/Living_Projects_TODO_Integration_Plan.md
        Status: Active - LP-001 (backend complete, UI integration pending)
        """
        if not project_name:
...
• Changes take effect on next training session"""
    ...
    def _apply_change_watcher_from_ui(self, initial: bool = False) -> None:
        ...
        if self.change_watcher_settings['enabled']:
            ...
            # Set version context for Code Watcher (GUI-V1)
            try:
                if VERSION_MANAGER_AVAILABLE:
                    version_manager = VersionManager()
                    active_version = version_manager.get_active_version()
                    if active_version:
                        watcher_set_active_version(active_version.version_id)
                        log_message(f"SETTINGS: Code Watcher version context set to {active_version.version_id}")
                    else:
                        watcher_set_active_version(None)
                        log_message("SETTINGS: No active version found, Code Watcher version context cleared")
            except Exception as exc:
                log_message(f"SETTINGS: Failed to set Code Watcher version context: {exc}")
...
• Regular evaluations to monitor skill drift"""
    ...
    def _clear_planner_dock_frames(self) -> None:
        content_pane = getattr(self, '_todo_content_pane', None)
        if not content_pane:
            self._planner_dock_frame = None
            return
        # Remove tracked frame
        existing = getattr(self, '_planner_dock_frame', None)
        if existing is not None:
            try:
                content_pane.forget(existing)
            except Exception:
                pass
            try:
                existing.destroy()
                ...
                pass
        self._planner_dock_frame = None
        # Remove any stray planner panes
        try:
            for pane_id in list(content_pane.panes()):
                widget = self.root.nametowidget(pane_id)
                if getattr(widget, '_is_planner_dock', False):
                    try:
                        content_pane.forget(widget)
                    except Exception:
                        pass
                    try:
                        widget.destroy()
    ...
    def _configure_todo_pane_sashes(self, has_planner_agent: bool, dock_visible: bool) -> None:
        content_pane = getattr(self, '_todo_content_pane', None)
        if not content_pane:
            return
        try:
            content_pane.update_idletasks()
        except Exception:
            return
        panes = content_pane.panes()
        if len(panes) < 2:
            return
        total_height = content_pane.winfo_height()
        if total_height < 200:
            try:
                total_height = max(400, content_pane.master.winfo_height() - 100)
            except Exception:
                total_height = 600
        tree_min = 220
        details_min = 220
        dock_min_expanded = 260
        dock_min_collapsed = 40

        desired_dock = 0
        if has_planner_agent:
            desired_dock = dock_min_expanded if dock_visible else dock_min_collapsed
        max_dock = max(0, total_height - (tree_min + details_min))
        dock_height = min(desired_dock, max_dock)
        if not dock_visible:
            dock_height = min(dock_height, dock_min_collapsed)
        remaining = max(tree_min + details_min, total_height - dock_height)

        tree_target = max(tree_min, int(total_height * 0.4))
        if tree_target > remaining - details_min:
            tree_target = max(tree_min, remaining - details_min)
        details_height = max(details_min, remaining - tree_target)
        details_target = tree_target + details_height
        try:
            content_pane.sash_place(0, 0, tree_target)
            if len(panes) >= 3:
                content_pane.sash_place(1, 0, details_target)
        except Exception:
            pass

    def _render_planner_dock(self, project_name: Optional[str]) -> bool:
        log_message("TODO POPUP: Rendering planner dock pane...")
        if not getattr(self, 'todo_popup_active', False):
            return False
        content_pane = getattr(self, '_todo_content_pane', None)
        if not content_pane:
            return False
        has_planner_agent = bool(self._resolve_planner_roster_entry())
        if not has_planner_agent:
            log_message("TODO POPUP: No planner agent published - dock hidden")
            self._clear_planner_dock_frames()
            self._planner_dock_visible = False
            self._configure_todo_pane_sashes(False, False)
            return True
        project_ctx = project_name or getattr(self, '_planner_project_context', None) or getattr(self, 'current_project_context', None)
        self._planner_project_context = project_ctx
        self._clear_planner_dock_frames()
        if getattr(self, '_planner_dock_visible', False):
            dock_pane = ttk.Frame(content_pane)
            dock_pane._is_planner_dock = True  # type: ignore[attr-defined]
            dock_frame = self._create_planner_dock_section(dock_pane, project_context=project_ctx)
            dock_frame.pack(fill=tk.BOTH, expand=True)
            expanded_height = max(320, int(content_pane.winfo_height() * 0.3) or 320)
            content_pane.add(dock_pane, minsize=expanded_height)
            self._planner_dock_frame = dock_pane
            log_message("TODO POPUP: Planner dock EXPANDED")
        else:
            dock_pane = ttk.Frame(content_pane, height=30)
            dock_pane._is_planner_dock = True  # type: ignore[attr-defined]
            dock_pane.pack_propagate(False)
            expand_btn = ttk.Button(
                dock_pane,
                text='▼ Expand Planner Agent Dock ▼',
                style='Action.TButton',
                command=lambda ctx=project_ctx: self._set_planner_dock_visibility(True, ctx)
            )
            expand_btn.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=4, pady=2)
            content_pane.add(dock_pane, minsize=36)
            self._planner_dock_frame = dock_pane
            log_message("TODO POPUP: Planner dock COLLAPSED (agent available)")
        self._configure_todo_pane_sashes(True, getattr(self, '_planner_dock_visible', False))
        return True
