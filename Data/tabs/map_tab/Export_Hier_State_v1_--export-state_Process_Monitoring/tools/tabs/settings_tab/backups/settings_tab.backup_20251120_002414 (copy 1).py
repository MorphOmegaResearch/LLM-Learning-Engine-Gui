...
try:
    from debug_logger import get_logger, debug_ui_event as _debug_ui_event
    _settings_tab_logger = get_logger("SettingsTab")
except ImportError:
    _debug_ui_event = None
    _settings_tab_logger = None
...
class SettingsTab(BaseTab):
    ...
    _settings_tab_debug_logger = get_debug_logger("settings_tab")
    ...
    def _collect_available_plans(self) -> List[Dict[str, Any]]:
        ...
        @debug_ui_event(_settings_tab_debug_logger)
    ...
    @_debug_ui_event(_settings_tab_logger)
...
'''
    ...
    def show_todo_popup(self, project_name: str = None, prefer_project: bool = False, auto_open_create: bool = False):
        ...
        # Only show if planner agent is configured in Agents tab
        log_message("TODO POPUP: Creating planner dock pane...")

        # PHASE SUB-ZERO-D: Check if planner agent is configured with variant selected
        has_planner_agent = False
        try:
            root = self.root
            if hasattr(root, 'custom_code_tab') and hasattr(root.custom_code_tab, 'agents_tab'):
                agents_tab = root.custom_code_tab.agents_tab
                if hasattr(agents_tab, 'agent_configs'):
                    log_message(f"TODO POPUP: Checking {len(agents_tab.agent_configs)} agent configs for planner")
                    # Check all agent configs for assigned_type == 'planner'
                    for aid, cfg in agents_tab.agent_configs.items():
                        assigned_type = cfg.get('assigned_type') or (agents_tab.get_agent_base_type(aid) if hasattr(agents_tab, 'get_agent_base_type') else None)
                        log_message(f"TODO POPUP: Agent '{aid}' has assigned_type='{assigned_type}'")

                        if assigned_type == 'planner':
                            # Check if variant is selected or override is set
                            variant = (cfg.get('variant') or '').strip()
                            has_override = cfg.get('gguf_override') or cfg.get('ollama_tag_override')
                            log_message(f"TODO POPUP: Found planner agent '{aid}' - variant='{variant}', has_override={bool(has_override)}")

                            if (variant and variant.lower() != 'not selected') or has_override:
                                has_planner_agent = True
                                log_message(f"TODO POPUP: ✓ Planner agent '{aid}' configured, will show dock toggle")
                                break
                            else:
                                log_message(f"TODO POPUP: ✗ Planner agent '{aid}' exists but no variant selected")
                            break  # Found planner agent, stop searching

                    if not has_planner_agent:
                        log_message("TODO POPUP: ✗ No active planner agent found in agent_configs")
                else:
                    log_message("TODO POPUP: agents_tab has no agent_configs attribute")
            else:
                log_message("TODO POPUP: Could not access agents_tab")
        except Exception as e:
            log_message(f"TODO POPUP: Error checking for planner agent: {e}")
            if _settings_debug_logger:
                _settings_debug_logger.error("Failed to check planner config", exc_info=e)
        ...
        # ===== ROW 4: PLANNER DOCK TOGGLE (conditional) =====
        if has_planner_agent:
            try:
                def _toggle_planner_dock():
                    """Toggle planner dock expansion state."""
                    self._planner_dock_visible = not getattr(self, '_planner_dock_visible', False)
                    log_message(f"TODO POPUP: Toggling planner dock to {self._planner_dock_visible}")

                    # Close and reopen popup to refresh dock state
                    if hasattr(self, '_active_todo_popup') and self._active_todo_popup:
                        try:
                            active_popup = self._active_todo_popup
                            project_name = getattr(self, '_active_popup_project', None)
                            active_popup.destroy()
                            log_message("TODO POPUP: Closed for dock toggle")
                        except Exception as e:
                            log_message(f"TODO POPUP: Error closing for toggle: {e}")
                        ...
                        try:
                            if project_name:
                                ...
                                self.show_main_todo_popup()
                            log_message(f"TODO POPUP: Reopened with dock_visible={self._planner_dock_visible}")
                        except Exception as e:
                            log_message(f"TODO POPUP: Error reopening: {e}")

                planner_toggle_row = ttk.Frame(frame)
                planner_toggle_row.grid(row=4, column=0, columnspan=2, sticky=tk.EW, pady=(4, 0))
                frame.rowconfigure(4, weight=0)

                toggle_text = "▲ Collapse Planner Dock" if dock_visible else "▼ Expand Planner Dock"
                ttk.Button(
                    planner_toggle_row,
                    text=toggle_text,
                    style='Action.TButton',
                    command=_toggle_planner_dock
                ).pack(fill=tk.X, padx=0, pady=0)

                log_message(f"TODO POPUP: Planner toggle row created (visible={dock_visible})")
            except Exception as e:
                log_message(f"TODO POPUP: Planner toggle row error: {e}")
                if _settings_debug_logger:
                    _settings_debug_logger.error("Failed to create planner toggle row", exc_info=e)
