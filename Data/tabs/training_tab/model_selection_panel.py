    ...
    def display_model_info(self, event=None):
        ...
        try:
            for m in get_all_trained_models():
                if m.get('name') == selected_model_name and m.get('type') in ('pytorch', 'trained', 'gguf'):
                    model_meta = m
                    break
        except Exception:
            model_meta = None
        ...
        if model_meta:
            _mtype = model_meta.get('type', '')
            _type_label = 'GGUF' if _mtype == 'gguf' else 'Trained' if _mtype == 'trained' else 'Base (PyTorch)'
            lines.append(f"Type: {_type_label}")
