...
try:
    ...
    # Soft fallback: allow import to succeed so probes/embedders don't kill the host app.
    # The class will raise at instantiation time if deps are truly missing.
    print(f"WARNING: ag_onboarding.py missing dependencies: {e}", file=sys.stderr)
    CLIWorkflow = None
    ConfigManager = None
    OllamaManager = None
    stream_tsv_file = None
    filter_by_focus = None
    parse_metadata = None
    populate_ag_forge_data = None
    KnowledgeForgeApp = None
...
class OnboardingReviewWorkflow:
    ...
    """
    def __init__(self, config, agent_mode: bool = False):
        if ConfigManager is None:
            raise ImportError("ag_onboarding dependencies not available (missing toml or other package)")
        self.config = config
