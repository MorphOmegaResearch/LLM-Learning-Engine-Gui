...
class PlannerSuite(tk.Frame):
    ...
    def load_directory_structure(self):
        ...
        for dir_name in ["Epics", "Plans", "Phases", "Tasks", "Milestones", "Diffs", "Refs"]:
            ...
            dir_id = self.tree.insert(root, "end", text=dir_name, values=("Folder", ""))
            
            # Store directory ID for toggling
            self.tree.set(dir_id, "dir_path", dir_path)
        ...
        try:
            ...
            for item in sorted(os.listdir(dir_path)):
                ...
                if os.path.isfile(item_path):
                    ...
                    self.tree.set(file_id, "file_path", item_path)
...
if __name__ == "__main__":
    root = tk.Tk()
    root.title("Planner Suite")
    root.geometry("1200x800")
    
    # Create the planner suite
    planner = PlannerSuite(root)
    planner.pack(fill=tk.BOTH, expand=True)
