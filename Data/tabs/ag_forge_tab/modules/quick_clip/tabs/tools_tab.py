...
class ToolsTab(ttk.Frame):
    ...
    def load_tools(self):
        if not os.path.exists(self.tools_file):
            ...
            default_tools = [
                ...
                {
                    "name": "Draft Ag Entity",
                    "command": "python ../bridge_staging.py --content \"{content}\" --json \"{json}\"",
                    "description": "Push extracted entity data to Ag Forge Staging."
                }
            ]
    ...
    def execute_command(self, command):
        try:
            # Replace placeholders with clipboard content
            import pyperclip
            content = pyperclip.paste()
            
            # Sanitize content for shell (simple escaping)
            safe_content = content.replace('"', '\\"')
            
            command = command.replace("{content}", safe_content)
            
            # For {json}, try to validate it first or just pass as is
            command = command.replace("{json}", safe_content)
