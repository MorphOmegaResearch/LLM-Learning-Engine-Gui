...
class ChatTab(ttk.Frame):
    ...
    def create_onboarding_prompt(self):
        #[Mark:UNIVERSAL_CATALOG_ONBOARDING]
        # Enhanced with ag_onboarding data analysis when available
        state = self.project_state

        # Try to enrich prompt with ag_onboarding's data/script analysis
        opportunities_text = ""
        try:
            from modules.ag_onboarding import OnboardingReviewWorkflow
            from modules.quick_clip.config import ConfigManager
            config = ConfigManager()
            workflow = OnboardingReviewWorkflow(config, agent_mode=True)
            script_caps = workflow._analyze_scripts()
            data_pots = workflow._analyze_data()
            all_opts = script_caps + data_pots
            if all_opts:
                opportunities_text = "\n".join(f"- {opt} (from {src})" for opt, src in all_opts)
                opportunities_text = f"\n\nAUTO-DETECTED OPPORTUNITIES:\n{opportunities_text}"
        except Exception:
            pass  # Fallback to basic prompt
...
2. If Focus missing -> Ask. If opportunities detected, suggest them.
...
Respond ONLY with your message for the user."""
    ...
    def call_ai(self, prompt, is_onboarding=False):
        ...
        if not get_provider:
            self.append_to_chat("System", "Error: Provider system unavailable — check imports.")
            return

        # Build args safely — app_ref may be _AppRef shim or full ClipboardAssistant
        class Args: pass
        args = Args()
        args.model = model_name
        args.ollama_url = getattr(self.app, 'ollama_url', 'http://localhost:11434/api/generate')
        args.gemini_api_key = self.config.get('Endpoints', 'gemini_api_key', '') if self.config else ''
        args.agent_mode = self.agent_mode_var.get()

        # Resolve the Tk root for thread callbacks
        _root = getattr(self.app, 'root', None)
        if _root is None:
            self.append_to_chat("System", "Error: No Tk root available for async callbacks.")
            return
        ...
        def run():
            try:
                ...
                if not provider:
                    _root.after(0, lambda: self.append_to_chat("System", f"Error: Provider '{provider_name}' not found."))
                    return

                response = provider.execute(prompt, args)
                _root.after(0, lambda: self.handle_ai_response(response, is_onboarding))
            except Exception as e:
                err = str(e)
                _root.after(0, lambda: self.append_to_chat("System", f"AI Error: {err}"))
            finally:
                _root.after(0, lambda: self.send_btn.config(state=tk.NORMAL))
