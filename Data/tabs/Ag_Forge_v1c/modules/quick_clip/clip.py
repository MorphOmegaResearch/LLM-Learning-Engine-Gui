...
from tabs.planner_tab import PlannerSuite
from tabs.tools_tab import ToolsTab
from tabs.state_manager_tab import StateManagerTab
...
try:
    ...
    print("DEBUG: Interactive Debug module imported successfully (QuickClip).", file=sys.stderr)
except ImportError as e:
    print(f"DEBUG ERROR: Could not import interactive_debug (QuickClip): {e}", file=sys.stderr)
    install_debug_hooks = None

class ClipboardAssistant:
    def __init__(self, root, geometry_override=None):
        self.root = root
        
        # Enable Debug Hooks if available
        if install_debug_hooks:
            install_debug_hooks(self.root)
        ...
        # Set window size from config or override
        ...
        self.ollama_url = self.config.get('Endpoints', 'ollama_url', 'http://localhost:11434/api/generate')
        ...
        self.create_chat_tab() # Added Chat Tab
    ...
    def set_resource_profile(self, profile_name):
        """Sets the number of GPU layers based on a selected profile."""
        if profile_name == 'max_gpu':
            layers = self.config.get('ResourceProfiles', 'max_gpu_layers', '999')
        elif profile_name == 'balanced':
            layers = self.config.get('ResourceProfiles', 'balanced_layers', '25')
        elif profile_name == 'cpu_only':
            layers = self.config.get('ResourceProfiles', 'cpu_only_layers', '0')
        else:
            return # Do nothing for unknown profiles
        
        self.num_gpu_var.set(layers)
        print(f"Set resource profile to '{profile_name}', GPU layers: {layers}")
    ...
    def setup_menus(self):
        ...
        project_menu.add_command(label="✨ New Agribusiness Project", command=self.start_agri_onboarding)
        
        # Tools menu
        tools_menu = tk.Menu(menubar, tearoff=0)
        menubar.add_cascade(label="Tools", menu=tools_menu)
        tools_menu.add_command(label="🔄 Fetch Ollama Models", command=self.fetch_models)
        tools_menu.add_command(label="🔗 Test AI Connection", command=self.test_connection)

    def start_agri_onboarding(self):
        """Redirect to the Chat tab and start onboarding"""
        self.notebook.select(self.chat_frame)
        self.chat_tab.start_onboarding()

    def create_state_tab(self):
        """Create the State Manager tab"""
        self.state_frame = ttk.Frame(self.notebook)
        self.notebook.add(self.state_frame, text='🗄️ State')
        
        # Embed the StateManagerTab
        state_tab = StateManagerTab(self.state_frame, app_ref=self)
        state_tab.pack(fill='both', expand=True)
    ...
    def create_tools_tab(self):
        ...
        # Embed the ToolsTab
        tools_tab = ToolsTab(self.tools_frame)
        tools_tab.pack(fill='both', expand=True)
    ...
    def create_chat_tab(self):
        ...
        self.chat_tab = ChatTab(self.chat_frame, app_ref=self)
    ...
    def create_planner_tab(self):
        ...
        self.notebook.add(self.planner_frame, text='📝 Planner')
        
        # Embed the PlannerSuite, passing a reference to this main app instance
        planner_suite = PlannerSuite(self.planner_frame, app_ref=self)
        planner_suite.pack(fill='both', expand=True)
    ...
    def create_config_tab(self):
        ...
        main_container = ttk.Frame(self.config_frame, padding=10)
        main_container.pack(fill='both', expand=True)
        main_container.columnconfigure(0, weight=1)
        
        # --- Service Endpoints ---
        endpoints_frame = ttk.LabelFrame(main_container, text="Service Endpoints", padding=15)
        endpoints_frame.grid(row=0, column=0, sticky='ew', pady=(0, 10))
        endpoints_frame.columnconfigure(1, weight=1)

        ttk.Label(endpoints_frame, text="Ollama URL:").grid(row=0, column=0, sticky='w', pady=5)
        self.url_var = tk.StringVar(value=self.config.get('Endpoints', 'ollama_url', 'http://localhost:11434/api/generate'))
        ttk.Entry(endpoints_frame, textvariable=self.url_var).grid(row=0, column=1, padx=10, pady=5, sticky='ew')

        ttk.Label(endpoints_frame, text="Gemini API Key:").grid(row=1, column=0, sticky='w', pady=5)
        self.gemini_key_var = tk.StringVar(value=self.config.get('Endpoints', 'gemini_api_key', ''))
        ttk.Entry(endpoints_frame, textvariable=self.gemini_key_var).grid(row=1, column=1, padx=10, pady=5, sticky='ew')

        # --- Resource Allocation ---
        resource_frame = ttk.LabelFrame(main_container, text="Resource Allocation", padding=15)
        resource_frame.grid(row=1, column=0, sticky='ew', pady=(0, 10))
        resource_frame.columnconfigure(1, weight=1)

        ttk.Label(resource_frame, text="Resource Profile:").grid(row=0, column=0, sticky='w', pady=5)
        profile_btn_frame = ttk.Frame(resource_frame)
        profile_btn_frame.grid(row=0, column=1, sticky='ew', padx=10, pady=5)
        ttk.Button(profile_btn_frame, text="Max GPU", command=lambda: self.set_resource_profile('max_gpu')).pack(side=tk.LEFT, padx=2)
        ttk.Button(profile_btn_frame, text="Balanced", command=lambda: self.set_resource_profile('balanced')).pack(side=tk.LEFT, padx=2)
        ttk.Button(profile_btn_frame, text="CPU Only", command=lambda: self.set_resource_profile('cpu_only')).pack(side=tk.LEFT, padx=2)

        ttk.Label(resource_frame, text="Advanced: GPU Layers (`num_gpu`):").grid(row=1, column=0, sticky='w', pady=5)
        self.num_gpu_var = tk.StringVar(value=self.config.get('Ollama', 'num_gpu', ''))
        ttk.Entry(resource_frame, textvariable=self.num_gpu_var, width=10).grid(row=1, column=1, sticky='w', padx=10, pady=5)
        
        # --- Ollama Model Settings ---
        ollama_frame = ttk.LabelFrame(main_container, text="Ollama Model", padding=15)
        ollama_frame.grid(row=2, column=0, sticky='ew', pady=(0, 10))
        ollama_frame.columnconfigure(1, weight=1)
        
        ttk.Label(ollama_frame, text="Model:").grid(row=0, column=0, sticky='w', pady=5)
        self.model_var = tk.StringVar(value=self.current_model)
        self.model_combo = ttk.Combobox(ollama_frame, textvariable=self.model_var, values=self.models, state='readonly')
        self.model_combo.grid(row=0, column=1, padx=10, pady=5, sticky='ew')
        self.model_combo.bind('<<ComboboxSelected>>', self.update_model)
        
        model_btn_frame = ttk.Frame(ollama_frame)
        model_btn_frame.grid(row=1, column=1, sticky='w', padx=10, pady=10)
        ttk.Button(model_btn_frame, text="🔗 Test Connection", command=self.test_connection).pack(side=tk.LEFT, padx=5)
        ttk.Button(model_btn_frame, text="🔄 Fetch Available Models", command=self.fetch_models).pack(side=tk.LEFT, padx=5)
        self.connection_label = ttk.Label(ollama_frame, text="", foreground="gray")
        self.connection_label.grid(row=2, column=0, columnspan=2, pady=5, sticky='w')

        # --- General ---
        general_frame = ttk.LabelFrame(main_container, text="General", padding=15)
        general_frame.grid(row=3, column=0, sticky='ew', pady=(0, 10))
        general_frame.columnconfigure(1, weight=1)

        ttk.Label(general_frame, text="Clipboard Refresh (sec):").grid(row=0, column=0, sticky='w', pady=5)
        self.refresh_var = tk.StringVar(value=self.config.get('Application', 'auto_refresh_seconds', '5'))
        ttk.Entry(general_frame, textvariable=self.refresh_var, width=10).grid(row=0, column=1, sticky='w', padx=10, pady=5)

        # --- Save Button ---
        save_btn_frame = ttk.Frame(main_container)
        save_btn_frame.grid(row=4, column=0, sticky='e', pady=10)
        ttk.Button(save_btn_frame, text="💾 Save All Settings", command=self.save_settings).pack()
    ...
    def save_settings(self):
        ...
        # Ollama settings
        self.config.set('Endpoints', 'ollama_url', self.url_var.get())
        self.config.set('Endpoints', 'gemini_api_key', self.gemini_key_var.get())
...
def main():
    ...
    Detects if running in CLI mode or GUI mode.
    """
    # --- CLI Mode ---
    parser = argparse.ArgumentParser(description="Quick Clip Assistant & Workflow Engine")
    
    # Workflow arguments
    parser.add_argument('--workflow', type=str, help="Path to a JSON workflow recipe file to execute.")
    
    # Direct-call arguments (can also be used by workflows if needed)
    parser.add_argument('--context-file', type=str, help="Path to a file to use as context.")
    parser.add_argument('--context-clipboard', action='store_true', help="Use the system clipboard as context.")
    parser.add_argument('--prompt', type=str, help="The prompt/instruction for the AI.")
    parser.add_argument('--prompt-file', type=str, help="Path to a file to use as the prompt.")
    parser.add_argument('--output-file', type=str, help="Path to save the final output.")
    parser.add_argument('--provider', type=str, default='ollama', help="Specify the AI provider (e.g., 'ollama', 'gemini').")
    parser.add_argument('--gpu-preset', type=str, choices=['low', 'medium', 'high'], help="Set a resource allocation profile.")
    
    # Execution
    parser.add_argument('--execute', action='store_true', help="Trigger the AI call or workflow execution.")
    parser.add_argument('--geometry', type=str, help="Window geometry (WxH+X+Y)")
    parser.add_argument('--passthrough', action='store_true', help="Enable direct agent-to-provider passthrough.")
    
    # Check if any arguments were passed.
    if len(sys.argv) > 1 and not any(arg.startswith('--geometry') for arg in sys.argv):
        args = parser.parse_args()
        # Pass all args to the handler, which will decide what to do.
        handle_cli_request(args)
        return # Exit after CLI execution

    # Need to re-parse if we skipped above, or if geometry was the only arg
    args, _ = parser.parse_known_args()

    # --- GUI Mode ---
    root = tk.Tk()
    app = ClipboardAssistant(root, geometry_override=args.geometry)
    ...
    try:
        ...
        if response.status_code != 200:
            messagebox.showwarning("Ollama Not Running", 
                                 ...
                                 "You can still use many features without AI.")
    except:
...
if __name__ == "__main__":
    # This block needs to be clean for the CLI to work well
    import sys
    import argparse
    
    # GUI-specific imports should be here or in the class
    import tkinter as tk
    ...
    from cli_handler import handle_cli_request
    from tabs.planner_tab import PlannerSuite
    from tabs.tools_tab import ToolsTab
    from tabs.state_manager_tab import StateManagerTab
    from config import ConfigManager
